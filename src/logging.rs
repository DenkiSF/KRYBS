...
pub fn init_logging(config: &CoreConfig) -> Result<()> {
    ...
    if let Some(parent) = Path::new(&config.log_file).parent() {
        if let Err(e) = std::fs::create_dir_all(parent) {
            eprintln!(
                "Предупреждение: не удалось создать каталог логов: {}. Логирование отключено.",
                e
            );
            return Ok(());
        }
    ...
    let roller = match FixedWindowRoller::builder().base(1).build(
        ...
        Err(e) => {
            eprintln!(
                "Предупреждение: не удалось настроить ротацию логов: {}. Логирование отключено.",
                e
            );
            return Ok(());
        }
    ...
    let appender = match appender {
        ...
        Err(e) => {
            eprintln!(
                "Предупреждение: не удалось инициализировать файл лога: {}. Логирование отключено.",
                e
            );
            return Ok(());
        }
        ...
        .build(Root::builder().appender("file").build(level))
    {
        ...
        Err(e) => {
            eprintln!(
                "Предупреждение: не удалось построить конфигурацию логгера: {}. Логирование отключено.",
                e
            );
            return Ok(());
        }
    ...
    if let Err(e) = log4rs::init_config(log_config) {
        eprintln!(
            "Предупреждение: не удалось инициализировать логгер: {}. Логирование отключено.",
            e
        );
    }
