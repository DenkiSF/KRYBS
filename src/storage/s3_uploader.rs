...
impl S3Uploader {
    ...
    pub async fn new(bucket: &str, region: &str, endpoint: Option<&str>) -> Result<Self> {
        let region_provider = RegionProviderChain::first_try(aws_sdk_s3::config::Region::new(region.to_string()))
            .or_default_provider()
    ...
    pub fn upload_backup(&self, backup_id: &str, local_dir: &Path, prefix: &str) -> Result<()> {
        ...
        let rt = tokio::runtime::Runtime::new().context("Не удалось создать Tokio runtime для S3-загрузки")?;

        for entry in WalkDir::new(local_dir).into_iter().filter_map(|e| e.ok()).filter(|e| e.file_type().is_file()) {
            let local_path = entry.path();
            let relative = local_path.strip_prefix(local_dir).expect("должен быть внутри корня");
            let object_key = format!("{}{}", prefix, relative.display());

            rt.block_on(self.upload_file(local_path, &object_key))?;
